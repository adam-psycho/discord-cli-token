...
func GetConfig() {
	...
	//Get File
	file, err := os.Open(usr.HomeDir + "/.config/discord-cli-token/config.json")
	if err != nil {
		log.Println(err)
		CreateConfig()
...
func CreateConfig() {
	...
	//Create Folder
	err = os.MkdirAll(usr.HomeDir+"/.config/discord-cli-token/", os.ModePerm)
	if err != nil {
		log.Fatalln(err)
	}

	//Create File
	file, err := os.Create(usr.HomeDir + "/.config/discord-cli-token/config.json")
	if err != nil {
		log.Fatalln(err)
	}
...
func CheckState() {
	...
	if Config.Token == "" {
		log.Fatalln("No Token Specified, please edit " + usr.HomeDir + "/.config/discord-cli-token/config.json")
	}
